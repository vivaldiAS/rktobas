@if($cek_service_booking)
    @foreach($service_booking_payment_validation as $service_book)
        <div class="card-body p-4">
            <div class="p-2 card shadow-0 border mb-1">
                ...
                <div class="row d-flex align-items-center">
                    @if($service_book->status == "on progress" || $service_book->status == "paid")
                        <div class="col-md-12">
                            <div class="d-flex justify-content-around mb-1">
                                @if($service_book->status == "on progress")
                                <p class="text-muted mt-1 mb-0 small ms-xl-5">PESANAN BERHASIL DIBAYARKAN. TUNGGU PERSETUJUAN ADMIN</p>
                                @endif

                                @if($service_book->status == "paid")
                                    <p class="text-muted mt-1 mb-0 small ms-xl-5">PEMBAYARAN DISETUJUI ADMIN</p>
                                @endif
                            </div>
                        </div>
                    @endif
